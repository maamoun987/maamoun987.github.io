...
if (window.location.href === 'https://ir-appointment.visametric.com/en/appointment-form') {
    $(document).ready(function () {
        ...
        if (typeof VA_CONFIG === 'undefined') {
			...
			if (event.status === 200 && cacheURLs[this.url]) {
			  caches[this.url] = event.responseText;
			} else if (event.status === 200 && !success.includes(this.url)) {
			  success.push(this.url);
			}
			if (this.url === `${baseUrl}/en/appointment-form/personal/getdate`) {
			  if (event.status === 200 && event.responseText === "Array\n(\n)\n") {
				refetchDate();
			  }
			  if (event.status === 200 && event.responseText !== "Array\n(\n)\n") {
				alert("Slot Available");
			  } else if (event.status === 502) {
				refetchDate();
			  } else if (event.status === 403) {
				if (confirm("Start again!") === true) {
				  setTimeout(() => {
					window.location.href = `${baseUrl}/en/`;
				  }, 1000);
				}
			  } else if (event.status === 429) {
				if (confirm("Change internet! and start again") === true) {
				  setTimeout(() => {
					window.location.href = `${baseUrl}/en/`;
				  }, 1000);
				}
